...
class TFNode {
    ...
    async choose() {
        ...
        // console.log(bonus.arraySync());
        bonus = tf.sqrt(bonus);
        
        bonus = tf.tensor([1.414]).mul(bonus);
        // console.log(bonus.arraySync())
...
class TFAgent {
    ...
    async mcts(node) {
        ...
        if (this.board.win) {
            ...
            value = -(await this.mcts(node.children[node.actions.indexOf(action)]));
        }
...
class Node {
    ...
    choose() {
        ...
        for (let i = 0; i < this.actions.length; i++) {
            scores.push(this.values[i] / this.counts[i] + 1.414 * Math.sqrt(2 * n / this.counts[i]));
        }
...
class Board {
    ...
    show() {
        ...
        current_game_board.style = "width: 384px; height:384px;";
        if (this.over) {
...
function human_play(board, index) {
    ...
    agent.play(index);
    board.show();
    if (!board.over) setTimeout(() => {computer_play(board).then(() => board.show())}, 16);
}

async function computer_play(board, action = null) {
    if (action == null) action = await agent.search();
    agent.play(action);
    return action;
}
